...
<script>
// 在这里引入的是没有办法在模板中使用的，所以还要在data中声明
import EmployeeEnum from '@/api/constant/employees'
import { getPersonalDetail, updatePersonal } from '@/api/employee'
import { getDetailInfo, saveUserDetailById } from '@/api/user'
import Img from '@/components/ImgUpload'
export default {
  components: {
    Img
  },
  ...
  methods: {
    ...
    async saveUser() {
      if (this.$refs.userInfoPhoto.fileList[0] && this.$refs.userInfoPhoto.fileList[0].status !== 'success') {
    ...
    async getUser() {
      this.userInfo = await getDetailInfo(this.$route.params.id)
