    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="转正评价">
          <el-input v-model="formData.correctionEvaluation" type="textarea" placeholder="1-300位字符" class="inputW" />
        </el-form-item>
      ...
      <div class="block">
        ...
        <el-form-item label="合同期限">
          <el-select v-model="formData.contractPeriod" class="filter-item inputW">
            <el-option
        ...
        <el-form-item label="续签次数">
          <el-select v-model="formData.renewalNumber" class="filter-item inputW">
            <el-option
      ...
      <div class="block">
        ...
        <el-form-item label="其他招聘渠道">
          <el-select v-model="formData.otherRecruitmentChannels" placeholder="请选择" class="inputW">
            <el-option
        ...
        <el-form-item label="招聘渠道">
          <el-select v-model="formData.recruitmentChannels" placeholder="请选择" class="inputW">
            <el-option
              v-for="item in EmployeeEnum.resumeSource"
              :key="item.value"
              :label="item.label"
        ...
        <el-form-item label="社招/校招">
          <el-select v-model="formData.socialRecruitment" placeholder="请选择" class="inputW">
            <el-option
        ...
        <el-form-item label="推荐企业/人" class="inputW">
          <el-input v-model="formData.recommenderBusinessPeople" placeholder="请输入" class="infoPosition inputW" />
        </el-form-item>
      </div>
      <!-- 从业信息 -->
      <el-form-item>
        <el-button type="primary" class="inputW" @click="saveJob">保存更新</el-button>
...
import { getJobDetail, updateJob, getSimpleList } from '@/api/employee'
export default {
  data() {
    return {
      EmployeeEnum,
  ...
  async created() {
    this.getJob()
    // this.saveJob()
    this.employeeList = await getSimpleList()
  },
  methods: {
    async saveJob() {
    ...
    async getJob() {
      this.formData = await getJobDetail(this.$route.params.id)
    }
  }
}
</script>
